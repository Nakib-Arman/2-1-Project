...
app.get("/showCart", authorization, async (req, res) => {
    try {
        const user_id = req.user;
        const cart = await pool.query("SELECT C.BOOK_ID, B.TITLE, B.CATEGORY, (SELECT PUBLICATION_NAME FROM PUBLISHERS WHERE PUBLISHER_ID = B.PUBLISHER_ID) AS PUBLICATION FROM CART C JOIN BOOKS B ON (C.BOOK_ID = B.BOOK_ID) WHERE b.IS_VISIBLE=TRUE AND C.USER_ID = $1", [user_id]);
        res.json(cart.rows);
    } catch (err) {
        console.error(err.message);
        res.status(500).json({ message: "Server Error" });
...
app.get("/publishers/:name",async (req,res) => {
    try{
        const {name} = req.params;
        const publisher = await pool.query("SELECT P.*, COUNT(B.BOOK_ID) AS BOOK_COUNT FROM PUBLISHERS P LEFT JOIN BOOKS B ON (P.PUBLISHER_ID=B.PUBLISHER_ID) WHERE B.IS_VISIBLE=TRUE GROUP BY P.PUBLISHER_ID HAVING LOWER(P.PUBLICATION_NAME) LIKE $1",[`%${name.toLowerCase()}%`]);
...
app.get("/showBooksByCategory", async (req, res) => {
    try {
        ...
        const books = await pool.query("SELECT BOOK_ID,B.IMAGE_URL,COPIES_AVAILABLE(BOOK_ID) COPY, TITLE, CATEGORY, (SELECT PUBLICATION_NAME FROM PUBLISHERS WHERE PUBLISHER_ID = B.PUBLISHER_ID) AS PUBLICATION FROM BOOKS B WHERE B.IS_VISIBLE=TRUE AND B.CATEGORY = $1 AND LOWER(B.TITLE) LIKE $2 ORDER BY BOOK_ID", [category, `%${title.toLowerCase()}%`]); // Adjusted query to use LIKE for case-insensitive title search
        res.json(books.rows);
    } catch (err) {
        console.error(err.message);
        res.status(500).json({ error: 'Internal Server Error' }); 
...
app.get("/studentacceptRequests/:id",async (req,res) =>{
    try{
        //const user_id=req.user;
        const students = await pool.query(
            "SELECT RBR.USER_REQUEST_ID AS REQUEST_ID,B.BOOK_ID,U.USER_ID AS STUDENT_ID,U.FIRST_NAME || ' ' || U.LAST_NAME AS NAME,B.TITLE AS TITLE,UR.REQUEST_DATE AS DATE_BORROWED, RBR.REQUEST_STATUS AS REQUEST_STATUS FROM USER_REQUEST UR JOIN USERS U ON(UR.USER_ID=U.USER_ID) JOIN REQUEST_BOOK_RELATION RBR ON (RBR.USER_REQUEST_ID= UR.USER_REQUEST_ID) JOIN BOOKS B ON (RBR.BOOK_ID = B.BOOK_ID) JOIN SHELVES S ON (S.SHELF_ID=B.SHELF_ID) JOIN USERS US ON(US.USER_ID=S.STAFF_ID) JOIN STUDENTS ST ON (ST.STUDENT_ID = U.USER_ID) WHERE B.IS_VISIBLE=TRUE AND US.USER_ID=$1 AND RBR.REQUEST_STATUS='Accepted'",[req.params.id]);
        res.json(students.rows);
    }catch (err) {
        console.error(err.message);
    }
...
app.get("/studentrejectedRequests/:id",async (req,res) =>{
    try{
        //const user_id=req.user;
        const students = await pool.query(
            "SELECT RBR.USER_REQUEST_ID AS REQUEST_ID,B.BOOK_ID,U.USER_ID AS STUDENT_ID,U.FIRST_NAME || ' ' || U.LAST_NAME AS NAME,B.TITLE AS TITLE,UR.REQUEST_DATE AS DATE_BORROWED, RBR.REQUEST_STATUS AS REQUEST_STATUS FROM USER_REQUEST UR JOIN USERS U ON(UR.USER_ID=U.USER_ID) JOIN REQUEST_BOOK_RELATION RBR ON (RBR.USER_REQUEST_ID= UR.USER_REQUEST_ID) JOIN BOOKS B ON (RBR.BOOK_ID = B.BOOK_ID) JOIN SHELVES S ON (S.SHELF_ID=B.SHELF_ID) JOIN USERS US ON(US.USER_ID=S.STAFF_ID) JOIN STUDENTS ST ON (ST.STUDENT_ID = U.USER_ID) WHERE B.IS_VISIBLE=TRUE AND US.USER_ID=$1 AND RBR.REQUEST_STATUS='Rejected'",[req.params.id]);
        res.json(students.rows);
    }catch (err) {
        console.error(err.message);
    }
...
app.get("/studentAllAccepted",async (req,res) =>{
    try{
        const students = await pool.query(
            "SELECT RBR.USER_REQUEST_ID AS REQUEST_ID,B.BOOK_ID,U.USER_ID AS STUDENT_ID,U.FIRST_NAME || ' ' || U.LAST_NAME AS NAME,B.TITLE AS TITLE,UR.REQUEST_DATE AS DATE_BORROWED, RBR.REQUEST_STATUS AS REQUEST_STATUS FROM USER_REQUEST UR JOIN USERS U ON(UR.USER_ID=U.USER_ID) JOIN REQUEST_BOOK_RELATION RBR ON (RBR.USER_REQUEST_ID= UR.USER_REQUEST_ID) JOIN BOOKS B ON (RBR.BOOK_ID = B.BOOK_ID) JOIN SHELVES S ON (S.SHELF_ID=B.SHELF_ID) JOIN USERS US ON(US.USER_ID=S.STAFF_ID) JOIN STUDENTS ST ON (ST.STUDENT_ID = U.USER_ID) WHERE B.IS_VISIBLE=TRUE AND RBR.REQUEST_STATUS='Accepted'"
        );
        res.json(students.rows);
    }catch(err){
        console.error(err.message);
...
app.get("/mypendingRequests/:user_id",async (req,res) => {
    try{
        ...
        const request = await pool.query(
            "SELECT * FROM USER_REQUEST UR JOIN REQUEST_BOOK_RELATION RBR ON (UR.USER_REQUEST_ID=RBR.USER_REQUEST_ID) JOIN BOOKS B ON (B.BOOK_ID=RBR.BOOK_ID) WHERE B.IS_VISIBLE=TRUE AND UR.USER_ID=$1 AND RBR.REQUEST_STATUS='Pending'",[user_id]
        )
...
app.get("/myacceptRequests/:user_id",async (req,res) => {
    try{
        const{user_id} = req.params;
        const request = await pool.query(
            "SELECT * FROM USER_REQUEST UR JOIN REQUEST_BOOK_RELATION RBR ON (UR.USER_REQUEST_ID=RBR.USER_REQUEST_ID) JOIN BOOKS B ON (B.BOOK_ID=RBR.BOOK_ID) JOIN USER_BORROW_RELATION UBR ON (UR.USER_ID=UBR.USER_ID AND UBR.BOOK_ID=RBR.BOOK_ID) WHERE B.IS_VISIBLE=TRUE AND UR.USER_ID=$1 AND RBR.REQUEST_STATUS='Accepted'",[user_id]
        )
        res.json(request.rows);
    }catch(err){
        console.error(err.message);
...
app.get("/myrejectedRequests/:user_id",async (req,res) => {
    try{
        const{user_id} = req.params;
        const request = await pool.query(
            "SELECT * FROM USER_REQUEST UR JOIN REQUEST_BOOK_RELATION RBR ON (UR.USER_REQUEST_ID=RBR.USER_REQUEST_ID) JOIN BOOKS B ON (B.BOOK_ID=RBR.BOOK_ID) WHERE B.IS_VISIBLE=TRUE AND USER_ID=$1 AND RBR.REQUEST_STATUS='Rejected'",[user_id]
        )
        res.json(request.rows);
    }catch(err){
        console.error(err.message);
...
app.put("/removeBook",async (req,res) =>{
    try{
        const {id} = req.body;
        const response = await pool.query("CALL DELETE_BOOK($1)",[id]);
        res.json(response);
    }catch(err){
        console.error(err.message);
    }
