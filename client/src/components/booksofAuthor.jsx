...
const BooksofAuthor = ({ setAuth }) => {
    ...
    const getAuthorName = async () => {
        try {
            const author = await fetch(`http://localhost:5000/authorName/${author_id}`);
            const authorData = await author.json();
            setAuthorName(authorData[0].author_name);
        } catch (err) {
    ...
    const getBooks = async () => {
        try {
            const response = await fetch(`http://localhost:5000/booksofAuthor/${author_id}`);
            const jsonData = await response.json();
            setBooks(jsonData);
            getAuthorName();
        } catch (err) {
    ...
    const searchBooks = async () => {
        try {
            console.log(title);
            const response = await fetch(`http://localhost:5000/bookofAuthor/${author_id}?title=${title}`);
            const jsonData = await response.json();
    ...
    return (
        <Fragment>
            <div className="page-container">
                ...
                <h1 className="fixed-header" style={{ backgroundColor: '#5A1917' }}>Books of {authorName}</h1>
