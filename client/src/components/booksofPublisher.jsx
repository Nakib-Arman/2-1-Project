...
const BooksofPublisher = ({ setAuth }) => {
    ...
    const getPublicationName = async () => {
        try {
            const publisher = await fetch(`http://localhost:5000/publicationName/${publisher_id}`);
            const publisherData = await publisher.json();
            console.log(publisherData[0].publication_name);
            setPublisherName(publisherData[0].publication_name);
        } catch (err) {
    ...
    const getBooks = async () => {
        try {
            const response = await fetch(`http://localhost:5000/booksofPublisher/${publisher_id}`);
            const jsonData = await response.json();
            setBooks(jsonData);
            getPublicationName();
        } catch (err) {
    ...
    const searchBooks = async () => {
        try {
            console.log(title);
            const response = await fetch(`http://localhost:5000/bookofPublisher/${publisher_id}?title=${title}`);
            const jsonData = await response.json();
    ...
    return (
        <Fragment>
            <div className="page-container">
                ...
                <h1 className="fixed-header" style={{ backgroundColor: '#5A1917' }}>Books of {publisherName}</h1>
...
export default BooksofPublisher;
